package com.github.damontecres.wholphin.services.compat
 ...
 * 2. Server hint ([dvP7Hint]): set when server pre-scan reports DOVI_WITH_EL.
...
class DvCompatVideoRenderer(
     ...
     *    In this case fall back to [dvP7Hint] which reflects the server's pre-scan result.
     */
    private fun isDvProfile7(format: Format?): Boolean {
        ...
        return when (format.sampleMimeType) {
            ...
            MimeTypes.VIDEO_H265 -> {
                ...
                // scanning won't find it. Use the server's authoritative signal instead.
                format.initializationData.any { bytes -> getDvccProfile(bytes) == 7 }
                    || dvP7Hint.get()
            }
            else -> false
